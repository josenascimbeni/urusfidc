...
import {
  BRAZIL_STATES,
  activeTemplateItems,
  additionalItemsFromTemplate,
  checklistItemsForFidc,
  createDocuments,
...
export default function Home() {
  ...
  function saveFidc() {
    ...
    const initialAdditionalItems = editingFidcId ? [] : additionalItemsFromTemplate(fidcForm.checklistTemplateId, templates);
    ...
    if (!editingFidcId) {
      setTemplates((items) => [...items, { id: `tpl-${savedId}`, name: `Adicionais de ${saved.name}`, scope: "FIDC", fidcId: savedId, status: "Ativo", activeVersion: 1, versions: [{ id: `tpl-${savedId}-v1`, version: 1, createdAt: "16/08/2026", createdBy: session?.id ?? "admin", items: initialAdditionalItems }] }]);
    }
  ...
  function saveChecklistItem(templateId: string, draft: ChecklistTemplateItem) {
    ...
    const standard = templates.find((template) => template.scope === "Padrão");
    if (target?.scope === "FIDC" && standard && activeTemplateItems(standard).some((item) => item.id === draft.id)) return notify("Itens herdados devem ser alterados no checklist padrão Urus.");
  ...
  function deleteChecklistItem(templateId: string, itemId: string) {
    setTemplates((items) => items.map((template) => {
      if (template.id !== templateId) return template;
      const active = template.versions.find((version) => version.version === template.activeVersion) ?? template.versions[0];
      if (!active.items.some((item) => item.id === itemId)) return template;
      const nextVersion = template.activeVersion + 1;
      return { ...template, activeVersion: nextVersion, versions: [...template.versions, { id: `${template.id}-v${nextVersion}`, version: nextVersion, createdAt: "16/08/2026", createdBy: session?.id ?? "admin", items: active.items.filter((item) => item.id !== itemId).map((item, index) => ({ ...item, order: index + 1 })) }] };
    }));
    notify("Documento excluído da próxima versão. Operações existentes preservam o checklist congelado.");
  }
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "checklist" && <AdminChecklist templates={templates} fidcs={fidcs} onSave={saveChecklistItem} onDelete={deleteChecklistItem} />}
      ...
      {showFidcForm && <FidcModal form={fidcForm} setForm={setFidcForm} error={fidcError} editing={Boolean(editingFidcId)} templates={templates} fidcs={fidcs} onClose={() => setShowFidcForm(false)} onSave={saveFidc} />}
    </main>
  );
}
...
/* Versão anterior preservada no histórico do Git.
...
*/
function AdminChecklist({ templates, fidcs, onSave, onDelete }: { templates: ChecklistTemplate[]; fidcs: FidcProfile[]; onSave: (templateId: string, item: ChecklistTemplateItem) => void; onDelete: (templateId: string, itemId: string) => void }) {
  const [templateId, setTemplateId] = useState(templates[0]?.id ?? "");
  const [draft, setDraft] = useState<ChecklistTemplateItem>({ ...EMPTY_CHECKLIST_ITEM });
  const template = templates.find((item) => item.id === templateId) ?? templates[0];
  const standard = templates.find((item) => item.scope === "Padrão") ?? templates[0];
  const inheritedItems = template?.scope === "FIDC" ? activeTemplateItems(standard) : [];
  const ownItems = template ? activeTemplateItems(template) : [];
  const combinedCount = template?.scope === "FIDC" ? checklistItemsForFidc(template.fidcId ?? "", templates).length : ownItems.length;
  const resetDraft = () => setDraft({ ...EMPTY_CHECKLIST_ITEM });
  const save = () => {
    if (!template || !draft.name.trim() || !draft.instructions.trim()) return;
    onSave(template.id, { ...draft, id: draft.id || `requirement-${Date.now()}`, detail: draft.detail || draft.name, expectedEvidence: draft.expectedEvidence.filter(Boolean) });
    resetDraft();
  };
  const changeTemplate = (nextId: string) => { setTemplateId(nextId); resetDraft(); };
  return <div className="page-content"><PageHeader eyebrow="ADMINISTRAÇÃO" title="Checklists por FIDC" description="Cada fundo recebe sempre o padrão Urus e pode exigir documentos adicionais próprios."/><div className="checklist-toolbar"><label>Configuração<select value={template?.id ?? ""} onChange={(event) => changeTemplate(event.target.value)}>{templates.map((item) => <option key={item.id} value={item.id}>{item.scope === "Padrão" ? "Padrão Urus" : fidcs.find((fidc) => fidc.id === item.fidcId)?.name ?? item.name}</option>)}</select></label><div><strong>Versão {template?.activeVersion} · {combinedCount} documentos</strong><small>{template?.scope === "FIDC" ? `${inheritedItems.length} herdados do padrão + ${ownItems.length} adicionais deste FIDC` : "Alterações no padrão valerão para todos os próximos checklists"}</small></div></div>{template?.scope === "FIDC" && <div className="inheritance-banner"><span>URUS</span><div><strong>Composição automática ativa</strong><p>Padrão Urus ({inheritedItems.length}) + adicionais de {fidcs.find((fidc) => fidc.id === template.fidcId)?.name} ({ownItems.length}).</p></div></div>}<div className="checklist-layout"><section className="panel checklist-admin"><div className="panel-heading"><div><h2>{template?.scope === "Padrão" ? "Documentos do padrão Urus" : "Checklist resultante"}</h2><p>Exclusões criam uma nova versão e não alteram operações em andamento.</p></div><button className="secondary-button compact" onClick={resetDraft}>＋ Incluir documento</button></div>{inheritedItems.length > 0 && <div className="checklist-section-label"><span>Padrão Urus obrigatório</span><small>Gerenciado na configuração “Padrão Urus”</small></div>}{inheritedItems.map((item) => <div className="checklist-row inherited" key={`inherited-${item.id}`}><span className="lock-mark">◆</span><span className="doc-icon">▱</span><span><strong>{item.name}</strong><small>{item.detail} · {item.multiplicity} · {item.maxSizeMb} MB</small></span><b>{item.required ? "Obrigatório" : "Não obrigatório"}</b><span className="origin-pill">Padrão Urus</span><span className="locked-action">Herdado</span></div>)}{template?.scope === "FIDC" && <div className="checklist-section-label additional"><span>Documentos adicionais do FIDC</span><small>Inclua somente exigências específicas deste fundo</small></div>}{ownItems.map((item) => <div className="checklist-row" key={item.id}><span className="drag">⠿</span><span className="doc-icon">▱</span><span><strong>{item.name}</strong><small>{item.detail} · {item.multiplicity} · {item.maxSizeMb} MB</small></span><b>{item.required ? "Obrigatório" : "Não obrigatório"}</b><button onClick={() => setDraft({ ...item, expectedEvidence: [...item.expectedEvidence] })}>Editar</button><button className="danger-link" onClick={() => { onDelete(template.id, item.id); if (draft.id === item.id) resetDraft(); }}>Excluir</button></div>)}{template?.scope === "FIDC" && ownItems.length === 0 && <button className="checklist-empty-action" onClick={resetDraft}><span>＋</span><strong>Incluir primeiro documento adicional</strong><small>O FIDC já recebe automaticamente todos os documentos do padrão Urus.</small></button>}</section><aside className="panel checklist-editor"><p className="eyebrow">{draft.id ? "EDITAR DOCUMENTO" : "INCLUIR DOCUMENTO"}</p><h2>{draft.id ? draft.name : template?.scope === "Padrão" ? "Novo item do padrão" : "Nova exigência do FIDC"}</h2><p className="editor-context">{template?.scope === "Padrão" ? "Será herdado por todos os FIDCs nos próximos casos." : "Será somado ao padrão Urus somente para este FIDC."}</p><Field label="Nome"><input value={draft.name} onChange={(event) => setDraft((item) => ({ ...item, name: event.target.value }))} placeholder="Ex.: Fluxo de caixa projetado"/></Field><Field label="Descrição"><input value={draft.detail} onChange={(event) => setDraft((item) => ({ ...item, detail: event.target.value }))} placeholder="Período ou escopo esperado"/></Field><Field label="Instruções"><textarea value={draft.instructions} onChange={(event) => setDraft((item) => ({ ...item, instructions: event.target.value }))} placeholder="Explique o que o documento deve conter"/></Field><div className="form-grid"><Field label="Multiplicidade"><select value={draft.multiplicity} onChange={(event) => setDraft((item) => ({ ...item, multiplicity: event.target.value as ChecklistTemplateItem["multiplicity"] }))}><option>Único</option><option>Por exercício</option><option>Por sócio</option></select></Field><Field label="Tamanho máximo (MB)"><input type="number" min="1" max="25" value={draft.maxSizeMb} onChange={(event) => setDraft((item) => ({ ...item, maxSizeMb: Number(event.target.value) }))}/></Field><Field label="Validade (dias)"><input type="number" value={draft.validityDays ?? ""} onChange={(event) => setDraft((item) => ({ ...item, validityDays: Number(event.target.value) || undefined }))}/></Field><Field label="Classificação do documento"><select value={draft.required ? "Obrigatório" : "Não obrigatório"} onChange={(event) => setDraft((item) => ({ ...item, required: event.target.value === "Obrigatório" }))}><option>Obrigatório</option><option>Opcional</option></select></Field></div><Field label="Evidências esperadas"><textarea value={draft.expectedEvidence.join(", ")} onChange={(event) => setDraft((item) => ({ ...item, expectedEvidence: event.target.value.split(",").map((value) => value.trim()) }))}/></Field><Field label="Padrão de validação da IA"><textarea value={draft.aiStandard} onChange={(event) => setDraft((item) => ({ ...item, aiStandard: event.target.value }))}/></Field><div className="draft-hint">A IA apenas recomenda. A decisão final continua sendo da Urus.</div><div className="form-actions"><button className="secondary-button" onClick={resetDraft}>Cancelar</button><button className="primary-button" disabled={!draft.name.trim() || !draft.instructions.trim()} onClick={save}>{draft.id ? "Salvar nova versão" : "Incluir documento"}</button></div></aside></div></div>;
}
...
function FidcModal({ form, setForm, error, editing, templates, fidcs, onClose, onSave }: { form: FidcProfile; setForm: React.Dispatch<React.SetStateAction<FidcProfile>>; error: string; editing: boolean; templates: ChecklistTemplate[]; fidcs: FidcProfile[]; onClose: () => void; onSave: () => void }) {
  const selectedOptions = (event: React.ChangeEvent<HTMLSelectElement>) => Array.from(event.target.selectedOptions, (option) => option.value);
  return <div className="modal-backdrop" role="presentation"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="fidc-modal-title"><header><div><p className="eyebrow">ADMINISTRAÇÃO DE MATCHING</p><h2 id="fidc-modal-title">{editing ? "Editar perfil do FIDC" : "Cadastrar novo FIDC"}</h2><p>Configure matching, endereço de distribuição e checklist.</p></div><button aria-label="Fechar" onClick={onClose}>×</button></header><div className="modal-body"><div className="form-grid"><Field label="Nome do FIDC" wide><input value={form.name} onChange={(event) => setForm((item) => ({ ...item, name: event.target.value }))} placeholder="Nome comercial"/></Field><Field label="E-mail de distribuição"><input type="email" value={form.email} onChange={(event) => setForm((item) => ({ ...item, email: event.target.value }))} placeholder="credito@fidc.com.br"/></Field><Field label="Status"><select value={form.status} onChange={(event) => setForm((item) => ({ ...item, status: event.target.value as FidcStatus }))}><option>Rascunho</option><option>Ativo</option><option>Inativo</option></select></Field>{!editing && <Field label="Checklist inicial" wide><select aria-label="Checklist inicial" value={form.checklistTemplateId} onChange={(event) => setForm((item) => ({ ...item, checklistTemplateId: event.target.value }))}><option value="tpl-standard">Padrão Urus · sem documentos adicionais</option>{templates.filter((template) => template.scope === "FIDC").map((template) => <option key={template.id} value={template.id}>Padrão Urus + {activeTemplateItems(template).length} adicional(is) de {fidcs.find((fidc) => fidc.id === template.fidcId)?.name ?? template.name}</option>)}</select><small>O novo FIDC sempre herdará o padrão Urus. Se escolher outro fundo, somente os documentos adicionais serão copiados.</small></Field>}<Field label="Regra de faturamento"><select value={form.revenueMode} onChange={(event) => setForm((item) => ({ ...item, revenueMode: event.target.value as RevenueMode }))}><option>Mínimo</option><option>Máximo</option><option>Faixa</option><option>Pontuação</option></select></Field><Field label="Valor de referência"><MoneyField value={form.minRevenue} onChange={(value) => setForm((item) => ({ ...item, minRevenue: value }))}/></Field>{form.revenueMode === "Faixa" && <Field label="Valor máximo"><MoneyField value={form.maxRevenue ?? 0} onChange={(value) => setForm((item) => ({ ...item, maxRevenue: value }))}/></Field>}<Field label="Peso do faturamento"><input type="number" min="0" max="100" value={form.weights.revenue} onChange={(event) => setForm((item) => ({ ...item, weights: { ...item.weights, revenue: Number(event.target.value) } }))}/></Field><Field label="Segmentos"><select multiple value={form.segments} onChange={(event) => setForm((item) => ({ ...item, segments: selectedOptions(event) }))}>{SEGMENTS.map((segment) => <option key={segment}>{segment}</option>)}</select><small>Use Cmd/Ctrl para selecionar vários.</small></Field><Field label="Regiões"><select multiple value={form.regions} onChange={(event) => setForm((item) => ({ ...item, regions: selectedOptions(event) }))}><option>Brasil</option>{BRAZIL_STATES.map((state) => <option key={state}>{state}</option>)}</select></Field><Field label="Tipos de operação" wide><select multiple value={form.operationTypes} onChange={(event) => setForm((item) => ({ ...item, operationTypes: selectedOptions(event) }))}>{OPERATION_TYPES.map((type) => <option key={type}>{type}</option>)}</select></Field></div><div className="required-toggle"><input id="revenue-required-new" type="checkbox" checked={form.revenueRequired} onChange={(event) => setForm((item) => ({ ...item, revenueRequired: event.target.checked }))}/><label htmlFor="revenue-required-new"><strong>Critério eliminatório</strong><small>Falha torna o FIDC inelegível; critérios preferenciais apenas pontuam.</small></label></div>{error && <div className="error-box">{error}</div>}<div className="draft-hint">Rascunhos não participam do matching. O checklist escolhido cria uma versão própria para o novo FIDC.</div></div><footer><button className="secondary-button" onClick={onClose}>Cancelar</button><button className="primary-button" onClick={onSave}>{editing ? "Salvar alterações" : "Cadastrar FIDC"}</button></footer></section></div>;
}
